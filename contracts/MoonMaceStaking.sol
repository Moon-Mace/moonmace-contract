...
contract MoonMaceStaking is Ownable2StepUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {

    // ============================= Variables =============================
    uint8 constant public MAX_DATE_SLOT = 10;

    MoonMaceToken public mcMON;

    // all the following variables are in the same decimal as mcMON (18 decimal)
    ...
    modifier onlyOperator {
        require(_msgSender() == operator, "MoonMace: caller is not the operator");
        _;
    }

    modifier allowNormalUnstake {
        require(featureFlags & _NORMAL_UNSTAKE_FLAG != 0, "MoonMace: normal unstake is not allowed");
        _;
    }
    
    modifier allowInstantUnstake {
        require(featureFlags & _INSTANT_UNSTAKE_FLAG != 0, "MoonMace: instant unstake is not allowed");
        _;
    }
    
    modifier allowClaim { 
        require(featureFlags & _CLAIM_FLAG != 0, "MoonMace: claim is not allowed");
        _;
    }
    ...
    function initialize(address _mcMONTokenAddress) public initializer {
        mcMON = MoonMaceToken(_mcMONTokenAddress);
        require(mcMON.decimals() == 18, "MoonMace: invalid mcMON token");
    ...
    function setStakeAssetCap(uint256 newTotalStakingCap) public onlyOwner {
        require(newTotalStakingCap >= totalStakingAmount, "MoonMace: staking cap too small");
    ...
    function setNormalUnstakeFee(uint256 newNormalUnstakeFee) public onlyOwner {
        require(newNormalUnstakeFee < 10000, "MoonMace: fee should be less than 100%");
    ...
    function setInstantUnstakeFee(uint256 newInstantUnstakeFee) public onlyOwner {
        require(newInstantUnstakeFee < 10000, "MoonMace: fee should be less than 100%");
    ...
    function collectFee() public onlyOwner {
        require(collectedFee > 0, "MoonMace: no collected fee");
     ...
     *  and then deposit X-Z to the contract. Meanwhile, the admin should request withdraw Y
     *  from the contract. `pendingStakeAmount` aims to record `X-Z`.
     */
    function withdraw() public nonReentrant onlyOperator {
        require(pendingStakeAmount > 0, "MoonMace: no pending stake amount");
    ...
    function deposit() public payable nonReentrant onlyOperator {
        ...
        require(amount > 0, "MoonMace: amount should be greater than 0");
    ...
    function withdrawFromInstantPool(uint256 amount) public nonReentrant onlyOperator {
        require(amount > 0, "MoonMace: amount should be greater than 0");
        require(amount <= instantPoolAmount, "MoonMace: insufficient instant pool amount");
    ...
    function unstakeRequest(uint256 amount) public nonReentrant whenNotPaused allowNormalUnstake {
        ...
        require(
            ...
            || pendingUnstakeAmount[user][slot] == 0, "MoonMace: claim the previous unstake first"
        );
    ...
    function claimSlot(uint8 slot) public nonReentrant whenNotPaused allowClaim {
        ...
        require(amount > 0, "MoonMace: no pending unstake");
        require(
    ...
    function claimAll() public nonReentrant whenNotPaused allowClaim {
        ...
        require(pendingCount > 0, "MoonMace: no pending unstake");   
        require(totalAmount > 0, "MoonMace: haven't reached the claimable time");
    ...
    function unstakeInstant(uint256 amount) public nonReentrant whenNotPaused allowInstantUnstake {
        ...
        require(amount > 0, "MoonMace: amount should be greater than 0");
        require(amount <= instantPoolAmount + pendingStakeAmount, "MoonMace: insufficient liquidity");
